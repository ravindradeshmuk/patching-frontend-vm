...
function TableComponent() {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        const response = await axios.get('http://localhost:3000/user/api/api/userdata'); // Fetch user data from your API endpoint
        const userData = response.data;

        // Assuming userData contains an array of objects with a 'team' property
        // Filter the data based on the user's team
        const userTeam = ''; // Replace with the actual user's team obtained from authentication
        const filteredData = userData.filter(item => item.team === userTeam);

        setTableData(filteredData);
        // Assuming the structure of table headings is consistent across all teams
        // You can set table headings based on the first item in the filtered data
        setTableHeadings(Object.keys(filteredData[0] || {}));
      } catch (error) {
  ...
  const renderColumn = (heading) => {
    const userTeam = ''; // Replace with the actual user's team obtained from authentication
    
    if (userTeam === 'scm team') {
      // Render only SCM App Group 2 column
      return heading === 'SCM App Validation';
    } else if (userTeam === 'monitoring team') {
      // Render only Monitoring Alerts Validation column
      return heading === 'Monitoring ISS/E-Link Validation'||'Monitoring Alerts Validation'||'Maintenance Mode Disabled';
    } else if (userTeam === 'db team') {
      // Render only DB Validation column
      return heading === 'DB Validation';
    } else if (userTeam === 'patching team') {
      // Render all columns except SCM App Group 2, Monitoring Alerts Validation, and DB Validation
      return !(heading === 'Monitoring ISS/E-Link Validation'||'Monitoring Alerts Validation'||'Maintenance Mode Disabled'||
        heading === 'DB Validation'||
        heading === 'SCM App Validation');
    }
    // If user's team is not defined, render all columns
    return true;
  };
  useEffect(() => {
...
const handleDropdownChange = async (event, rowIndex, columnName) => {
  ...
  try {
    const response = await fetch(`${apiDomain}/client/data/updateClientData/${rowId}`, {
      method: 'PATCH',
      headers: {
